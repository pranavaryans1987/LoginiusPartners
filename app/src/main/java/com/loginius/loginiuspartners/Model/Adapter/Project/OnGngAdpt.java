...
public class OnGngAdpt extends RecyclerView.Adapter<OnGngAdpt.ViewHolder> {
    Context context;
    List<Project> list;

    public OnGngAdpt(Context context, List<Project> list) {
    ...
    @NonNull
    @Override
    public OnGngAdpt.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.on_goning_list, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull OnGngAdpt.ViewHolder holder, int position) {
        holder.prjnm.setText(list.get(position).getPrj());
        holder.date.setText("DUE DATE :- "+list.get(position).getDate());
